    ...
    : JackWaiterDriver(name, alias, engine, table)
{
    ...
    audio_cycle_ns = ((float)period_size/sample_rate) * 1000000000;
    ...
    jack_error("Tmp Residue Sample Duration = %f", tmp_residueSamplesDuration);
...
int JackAVBDriver::Read()
{
    ...
    if ( cumulative_rx_int_ns > this->audio_cycle_ns ) {
        ret = 1;
        ...
        int dummy = 0;
        for(int i = 0; i < (this->audio_cycle_ns - cumulative_rx_int_ns); i++) 
            dummy *= 1.1;
    }
    
    this->monotonicTime += this->audio_cycle_ns;
